...
if (isset($_POST['login'])) {
    ...
    if (!empty($username) && !empty($password)) {
        ...
        // Menggunakan $conn sesuai dengan file koneksi Anda
        ...
        if ($result && mysqli_num_rows($result) === 1) {
            ...
            if (password_verify($password, $row['password'])) {
                // SINKRONISASI SESSION KETAT UNTUK PELANGGAN
                $_SESSION['role'] = 'pelanggan';
                $_SESSION['id_pelanggan'] = $row['id_pelanggan'];
                $_SESSION['nama_pelanggan'] = $row['nama']; // Disamakan dengan pemanggilan di index pelanggan
    ...
    <style>
        ...
        body {
            ...
            background: radial-gradient(circle at 10% 20%, rgba(216, 241, 230, 0.46) 0.1%, rgba(233, 226, 226, 0.28) 90.1%),
                        linear-gradient(120deg, #fdfbf7 0%, #eefcf6 100%);
            min-height: 100vh;
            display: flex;
            align-items: center;
        ...
        .bubble {
            position: absolute; border-radius: 50%; background: var(--primary-gradient); filter: blur(80px); opacity: 0.15; z-index: 0;
        }
        .bubble-1 { width: 350px; height: 350px; top: -10%; left: -10%; }
        .bubble-2 { width: 400px; height: 400px; bottom: -15%; right: -5%; }

        .login-container { z-index: 1; width: 100%; max-width: 450px; padding: 15px; }
    ...
    <div class="login-container">
        <div class="login-card p-5">
            <div class="text-center mb-4">
                ...
                <h3 class="brand-header fw-bold mb-1">INDOMAX CLIENT</h3>
                <p class="text-muted small">Area Login Pelanggan</p>
            </div>
            ...
            <form action="" method="POST">
                ...
                <button type="submit" name="login" class="btn btn-success w-100 btn-submit text-white mb-4">
                    Masuk Akun <i class="bi bi-arrow-right-short ms-1 fs-5"></i>
                </button>

                <div class="text-center">
                    <p class="small text-muted mb-1">Belum punya akun? <a href="register_pelanggan.php" class="text-success fw-bold text-decoration-none">Daftar Sekarang</a></p>
                    <p class="small text-muted mb-0">Pelanggan lama? <a href="aktivasi_pelanggan.php" class="text-primary fw-bold text-decoration-none">Aktivasi Akun di Sini</a></p>
                </div>
            </form>
        </div>
    </div>
